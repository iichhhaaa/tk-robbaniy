...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql_fetch = "SELECT foto FROM fasilitas WHERE id = ?";
    ...
    if ($_FILES["foto"]["name"] != "") {
        ...
        $target_dir = "../../../storage/fasilitas/"; // Directory to store the uploaded image
        
        // Generate a unique file name using uniqid() and rand()
        $file_extension = strtolower(pathinfo($_FILES["foto"]["name"], PATHINFO_EXTENSION));
        $unique_name = uniqid('fs', true) . rand(1000, 9999) . '.' . $file_extension;
    ...
    $sql_update = "UPDATE fasilitas SET nama = ?, foto = ? WHERE id = ?";

    if ($stmt_update = $conn->prepare($sql_update)) {
        // Bind the parameters
        $stmt_update->bind_param("ssi", $nama, $foto, $id);
